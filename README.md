...
However in this repo, instead of using PHP, i'm using CloudFunctions on GCP and Firestore DB to save and retrive the link assigned from Ngrok.

The two Python files in the directory are: one for open HTTPS connection with Ngrok and the other for a SSH connection. This programmes are made for use on Linux (tested on Ubuntu 22.04), and, of course Google Cloud Platform.

- A call of the read_data_http URL will automatically redirect on the server page shared with Ngrok
- A call of the read_data_ssh URL will print the shell command on the page to open the SSH connection (for a Mac Terminal)
...
```
#### 2. Update URLs inside Python files in root folder with the URL assigned by GCP to access at the save_data function
